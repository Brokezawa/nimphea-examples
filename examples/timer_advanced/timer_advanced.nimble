...
description   = "Nimphea Example: timer_advanced"
license       = "MIT"
srcDir        = "src"
bin           = @["timer_advanced"]
...
task flash, "Flash via DFU":
  exec "dfu-util -a 0 -s 0x08000000:leave -D build/timer_advanced.bin"

task stlink, "Flash via ST-Link":
  exec "openocd -f interface/stlink.cfg -f target/stm32h7x.cfg -c \"program build/timer_advanced.elf verify reset exit\""
