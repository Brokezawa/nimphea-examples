...
description   = "Nimphea Example: vu_meter"
license       = "MIT"
srcDir        = "src"
bin           = @["vu_meter"]
...
task flash, "Flash via DFU":
  exec "dfu-util -a 0 -s 0x08000000:leave -D build/vu_meter.bin"

task stlink, "Flash via ST-Link":
  exec "openocd -f interface/stlink.cfg -f target/stm32h7x.cfg -c \"program build/vu_meter.elf verify reset exit\""
