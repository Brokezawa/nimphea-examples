...
description   = "Nimphea Example: dac_simple"
license       = "MIT"
srcDir        = "src"
bin           = @["dac_simple"]
...
task flash, "Flash via DFU":
  exec "dfu-util -a 0 -s 0x08000000:leave -D build/dac_simple.bin"

task stlink, "Flash via ST-Link":
  exec "openocd -f interface/stlink.cfg -f target/stm32h7x.cfg -c \"program build/dac_simple.elf verify reset exit\""
