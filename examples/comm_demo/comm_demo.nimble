...
description   = "Nimphea Example: comm_demo"
license       = "MIT"
srcDir        = "src"
bin           = @["comm_demo"]
...
task flash, "Flash via DFU":
  exec "dfu-util -a 0 -s 0x08000000:leave -D build/comm_demo.bin"

task stlink, "Flash via ST-Link":
  exec "openocd -f interface/stlink.cfg -f target/stm32h7x.cfg -c \"program build/comm_demo.elf verify reset exit\""
