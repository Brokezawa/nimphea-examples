...
description   = "Nimphea Example: sai_demo"
license       = "MIT"
srcDir        = "src"
bin           = @["sai_demo"]
...
task flash, "Flash via DFU":
  exec "dfu-util -a 0 -s 0x08000000:leave -D build/sai_demo.bin"

task stlink, "Flash via ST-Link":
  exec "openocd -f interface/stlink.cfg -f target/stm32h7x.cfg -c \"program build/sai_demo.elf verify reset exit\""
