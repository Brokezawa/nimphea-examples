...
description   = "Nimphea Example: codec_comparison"
license       = "MIT"
srcDir        = "src"
bin           = @["codec_comparison"]
...
task flash, "Flash via DFU":
  exec "dfu-util -a 0 -s 0x08000000:leave -D build/codec_comparison.bin"

task stlink, "Flash via ST-Link":
  exec "openocd -f interface/stlink.cfg -f target/stm32h7x.cfg -c \"program build/codec_comparison.elf verify reset exit\""
