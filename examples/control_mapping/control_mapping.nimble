...
task flash, "Flash via DFU":
  exec "dfu-util -a 0 -s 0x08000000:leave -D build/control_mapping.bin"

task stlink, "Flash via ST-Link":
  exec "openocd -f interface/stlink.cfg -f target/stm32h7x.cfg -c \"program build/control_mapping.elf verify reset exit\""
